...
func TestNewSubagent(t *testing.T) {
	...
	if sub.maxLoop != subagentMaxLoop || sub.systemPrompt != subagentSystemPrompt {
...
func TestTaskTool_ReturnsSubagentFinalText(t *testing.T) {
	...
	if llm.systems[1].Content != subagentSystemPrompt || llm.systems[0].Content != defaultSystemPrompt {
