...
func TestCheckUserAllow(t *testing.T) {
	...
	for _, c := range cases {
		...
		if got := checkUserAllow(tool); got != c.want {
...
func TestCheckToolPermission(t *testing.T) {
	...
	promptIn = yesReader{}
	allowed, msg := checkToolPermission(MessagesBlock{Name: "run_bash", Input: map[string]interface{}{"command": "rm -rf ."}})
	...
	allowed, msg = checkToolPermission(MessagesBlock{Name: "run_bash", Input: map[string]interface{}{"command": "cat a.txt"}})
...
func TestCheckToolPermission_AllowListSkipsPrompt(t *testing.T) {
	...
	allowed, msg := checkToolPermission(MessagesBlock{Name: "run_bash", Input: map[string]interface{}{"command": "ls -la"}})
	if !allowed || msg != "" || out.Len() != 0 {
		t.Errorf("whitelisted command: allowed=%v msg=%q prompted=%q", allowed, msg, out.String())
	}

	allowed, msg = checkToolPermission(MessagesBlock{Name: "todo_write", Input: map[string]interface{}{"todos": []interface{}{}}})
	if !allowed || msg != "" || out.Len() != 0 {
	...
	allowed, msg = checkToolPermission(MessagesBlock{Name: "run_bash", Input: map[string]interface{}{"command": "ls; rm -rf /"}})
	if allowed || !strings.Contains(msg, "not allowed") {
		t.Errorf("chained command: allowed=%v msg=%q", allowed, msg)
	}

	// non-whitelisted command still reaches the prompt
	allowed, _ = checkToolPermission(MessagesBlock{Name: "run_bash", Input: map[string]interface{}{"command": "cat a.txt"}})
